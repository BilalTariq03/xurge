...
export class BookingAnimation extends AnimationBase{
  ...
  setupBookingAnimation(){
    ...
    this.mm.add("(max-width: 990px) and (min-width: 601px)", () => {
      ...
      const updatePadding = () => {
        bookingSection.style.paddingBottom = totalDuration() + 'px';
      };
      updatePadding();
      ScrollTrigger.addEventListener("refresh", updatePadding);
      ...
      return () => {
        ScrollTrigger.removeEventListener("refresh", updatePadding);
        bookingSection.style.paddingBottom = '';
        pinTrigger.kill();
